            ...
            for index, entry in enumerate(entries):
                ...
                try:
                    ...
                    if os.path.exists(full_path):
                        ...
                        if not is_dir:
                            size = stat.st_size
                            if self.count_lines and size < 2 * 1024 * 1024:  # 限制2MB以下文件统计行数
...
class FileBrowser(QWidget):
    ...
    # 信号
    path_changed = Signal(str)          # 路径变更
    file_selected = Signal(str)         # 文件选择
    file_double_clicked = Signal(str)   # 文件双击
    folder_created = Signal(str)        # 文件夹创建
    ...
    def __init__(self, root_path=None, parent=None):
        ...
        self.exclude_patterns = []  # 排除模式列表
        self.show_hidden = False
        self.count_lines = True  # 默认统计行数
        self._loading = False
        self._worker = None
    ...
    def _setup_ui(self):
        ...
        self.btn_generate_from_text.setToolTip("输入树状架构文本（支持根目录识别、缩进层级、│及#注释），自动生成项目结构")
        ...
        # 开启多选支持 (ExtendedSelection)[cite: 2]
        ...
        if obj == self.tree:
            if event.type() == QKeyEvent.KeyPress:
                ...
                if key == Qt.Key_Return or key == Qt.Key_Enter:
                    ...
                    if current:
                        self._on_item_double_clicked(current, 0)
                    return True
                    ...
                    # 支持通过键盘 Delete 键批量删除选中的多项内容[cite: 2]
    ...
    def _show_text_architecture_dialog(self):
        """弹出架构文本输入框，完美解析带缩进、根目录、│及#注释的树状架构"""
        ...
        tip_label = QLabel(
            "请在下方粘贴您的树状目录结构文本（支持顶级根目录、缩进层级、│ 及 # 注释）："
        )
    ...
    def _show_context_menu(self, position):
        ...
        # 绑定父对象 self 防止局部 QMenu 在执行前被垃圾回收
        menu = QMenu(self)
