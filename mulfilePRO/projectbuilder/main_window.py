...
# ---------- 导入代码合并工具（独立模块） ----------
from code_merger import CodeMergerWidget


class MainWindow(QMainWindow):
    """
    主窗口类，界面大小为显示器宽度的85%，高度为可用屏幕高度的90%
    包含：文件浏览器、代码差异、源码浏览、分批复制、代码合并
    """
    ...
    def _setup_geometry(self):
        """窗口宽度为屏幕宽度的85%，高度为可用屏幕高度的90%"""
        screen = QApplication.primaryScreen()
        if screen:
            avail = screen.availableGeometry()
            width = int(avail.width() * 0.85)
            height = int(avail.height() * 0.90)
    ...
    def _setup_ui(self):
        ...
        # 【新增】连接保存后返回文件浏览器的信号
        self.source_viewer_widget.switch_to_file_browser.connect(self._on_switch_to_file_browser)
        ...
        self.tab_widget.addTab(self.code_merger, "🧩 代码合并")
    ...
    def _on_code_merge_requested(self, path):
        ...
        if hasattr(self, 'code_merger'):
            self.code_merger.set_source_path(path)
            self.statusBar().showMessage(f"🧩 已切换到代码合并，源路径: {path}")
        else:
            self.statusBar().showMessage("⚠️ 代码合并模块未加载")

    # ---------- 源码浏览信号 ----------
    def _on_switch_to_file_browser(self):
        """保存文件后切换到文件浏览器Tab（勾选复选框时触发）"""
        self.tab_widget.setCurrentIndex(0)
        self.statusBar().showMessage("已返回文件浏览器")
