...
class FileBrowserController(QObject):
    ...
    # ---------- 新建 ----------
    def _on_folder_create(self, full_path):
        """
        创建文件夹
        - 如果传入的是完整路径，直接创建
        - 如果传入的是名称（兼容旧调用），在当前目录创建
        """
        # 检查传入的是完整路径还是名称
        if os.path.dirname(full_path) != full_path and os.sep in full_path:
            # 包含路径分隔符，视为完整路径
            ...
            # 仅名称，在当前目录创建
            name = full_path
            parent_dir = self.model.current_path
        ...
        if not name or not name.strip():
            ...
            if not ok or not name or not name.strip():
                return
            target_path = os.path.join(self.model.current_path, name.strip())
        
        # 检查路径是否已存在
        if os.path.exists(target_path):
            QMessageBox.warning(self.view, "错误", f"路径已存在:\n{target_path}")
            return
        
        try:
            os.makedirs(target_path)
            self._refresh_view()
            self.view.update_status(f"已创建文件夹: {os.path.basename(target_path)}")
        except Exception as e:
            QMessageBox.warning(self.view, "错误", f"创建文件夹失败:\n{str(e)}")

    def _on_file_create(self, full_path):
        """
        创建文件
        - 如果传入的是完整路径，直接创建
        - 如果传入的是名称（兼容旧调用），在当前目录创建
        ...
        try:
            with open(target_path, 'w', encoding='utf-8') as f:
                f.write("")
            self._refresh_view()
            self.view.update_status(f"已创建文件: {os.path.basename(target_path)}")
        except Exception as e:
            QMessageBox.warning(self.view, "错误", f"创建文件失败:\n{str(e)}")
