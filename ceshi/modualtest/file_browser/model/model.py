...
class FileBrowserModel(QObject):
    ...
    def delete_file(self, path):
        """删除文件或文件夹，返回是否成功"""
        print(f"[MODEL-DEBUG] delete_file 被调用, path: {path}")
        print(f"[MODEL-DEBUG] path 类型: {type(path)}")
        
        # 确保路径是绝对路径且规范化
        path = os.path.normpath(os.path.abspath(path))
        print(f"[MODEL-DEBUG] 规范化后路径: {path}")
        
        if not os.path.exists(path):
            print(f"[MODEL-DEBUG] 路径不存在: {path}")
            self.sig_error.emit(f"文件不存在: {path}")
            return False
        
        try:
            if os.path.isdir(path):
                print(f"[MODEL-DEBUG] 删除文件夹: {path}")
                shutil.rmtree(path)
            else:
                print(f"[MODEL-DEBUG] 删除文件: {path}")
                os.remove(path)
            print(f"[MODEL-DEBUG] 删除成功: {path}")
            self.sig_file_deleted.emit(path)
            return True
        except Exception as e:
            print(f"[MODEL-DEBUG] 删除失败: {path}, 错误: {str(e)}")
