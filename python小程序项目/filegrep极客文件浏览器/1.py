...
from PySide2.QtGui import (QTextCursor, QTextCharFormat, QColor, QSyntaxHighlighter, 
...
class FileViewer(QMainWindow):
    def __init__(self):
        ...
        # self.setWindowIcon(QIcon("firegrep.png"))
        # 修改后：通过 __file__ 获取当前脚本绝对路径，拼接出图标的绝对路径
    ...
    def save_config(self):
        try:
            self.config["file_extensions"] = self.edit_ext.text()
    ...
    def run_search_all_files(self):
        """点击全部文件搜索：获取当前命令，移除后缀参数后执行，控件命令保持不变"""
        current_cmd = self.edit_command.text().strip()
        # 利用正则过滤掉形如 --include="..." 的参数及其多余空格
    ...
    def update_command_preview(self, ignore_ext=False):
        ...
        if selected_exts:
            includes = " ".join([f'--include="*{ext if ext.startswith(".") else "." + ext}"' for ext in selected_exts])
    ...
    def get_current_keyword(self):
        ...
        if self.chk_custom_cmd.isChecked():
            try:
                ...
                potential = [arg for arg in parsed_args if not arg.startswith('-') and arg != 'grep' and arg != '.' and '*' not in arg]
    ...
    def execute_grep_command(self, command_str):
        """核心执行函数"""
        keyword = self.get_current_keyword()
        if self.chk_custom_cmd.isChecked():
            self.config["custom_command"] = command_str
            self.save_config()
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for line in lines:
                    ...
                    if should_highlight:
                        ...
                        label.setStyleSheet("font-family: monospace; font-size: 13px; background-color: #F1F8E9; border-radius: 4px;")
                    else:
                        label.setStyleSheet("font-family: monospace; font-size: 13px;")
