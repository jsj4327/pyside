# -*- coding:utf-8 -*-
"""
crawler_tab.py — 爬虫模块（支持日期范围抓取）
三阶段爬取：生成版面URL → 文章列表 → 正文抓取
存储引擎：SQLite (WAL + 批量事务 + URL唯一索引去重)
"""
import os
import re
import json
...
from PySide2.QtCore import QThread, Signal, Qt, QSettings, QTimer, QDate
import requests
...
class ArticleDB:
    ...
    def _init_db(self):
        ...
        conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_art_crawled ON articles(crawled_at);
    ...
    def create_session(self, start_url):
        conn = self._get_conn()
        cur = conn.execute(
            "INSERT INTO crawl_sessions (start_url) VALUES (?)", (start_url,)
        )
        conn.commit()
    ...
    def update_contents(self, articles):
        """批量更新正文"""
        conn = self._get_conn()
        conn.executemany(
            """UPDATE articles
    ...
    def get_stats(self):
        ...
        by_section = conn.execute(
            ...
            "WHERE section != '' GROUP BY section ORDER BY cnt DESC"
        ).fetchall()
        by_date = conn.execute(
            "SELECT pub_date, COUNT(*) as cnt FROM articles "
            "WHERE pub_date != '' GROUP BY pub_date ORDER BY pub_date DESC LIMIT 10"
        ).fetchall()
        return {
            'total': total,
    ...
    def export_txt(self, output_path, limit=None):
        conn = self._get_conn()
        sql = "SELECT * FROM articles ORDER BY section, id"
        params = []
        if limit:
            sql += " LIMIT ?"
            params.append(limit)
...
class CrawlerThread(QThread):
    ...
    batch_saved_signal = Signal(int, int)

    def __init__(self, base_url, start_date, end_date, db_path, fetch_content=True, batch_size=BATCH_SIZE):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            session_info = f"{self.base_url} | {self.start_date} ~ {self.end_date}"
            session_id = self._db.create_session(session_info)
            
            self.log_signal.emit(f"🚀 开始爬取: {session_info}", "info")
            self.log_signal.emit(f"💾 数据库: {self.db_path}", "info")
            self.log_signal.emit(f"📊 库中已有: {self._db.get_total_count()} 条", "info")

            # ===== 生成版面URL列表 =====
            layout_urls = self._generate_layout_urls()
            if not layout_urls:
                self._db.finish_session(session_id, 0, 0, 'empty')
                self.finished_signal.emit({"status": "empty"})
                return

            self.log_signal.emit(f"📅 共生成 {len(layout_urls)} 个版面链接", "info")
            ...
            for idx, layout_url in enumerate(layout_urls):
                if self._is_cancelled:
                    break
                self.progress_signal.emit(idx, total_layouts, "抓取文章列表")
                self.log_signal.emit(f"📰 [{idx+1}/{total_layouts}] 处理版面: {layout_url}", "info")
                
                html = self._fetch_page(layout_url)
                if not html:
                    continue
                
                # 从版面URL中提取日期，例如 .../layout/202607/30/... -> 2026-07-30
                layout_date = self._extract_date_from_layout_url(layout_url)
                
                # 解析该版面下的所有文章
                articles = self._parse_layout_page(html, layout_url, layout_date)
                
                # 去重合并
                for art in articles:
                    norm = urlparse(art['url'])._replace(fragment='').geturl()
                    if norm not in seen_urls:
                        seen_urls.add(norm)
                        all_articles.append(art)
                
                self.log_signal.emit(f"   📝 发现 {len(articles)} 篇", "info")
                time.sleep(random.uniform(0.3, 0.8))

            if self._is_cancelled:
                self._db.finish_session(session_id, len(all_articles), 0, 'cancelled')
            ...
            self.log_signal.emit(f"✅ 第一阶段完成: 共发现 {len(all_articles)} 篇文章", "success")

            # 批量写入列表数据
            total_inserted, total_skipped = self._batch_save(all_articles)
            self.log_signal.emit(
                f"💾 列表写入: 新增 {total_inserted}, 跳过 {total_skipped}",
                "success"
            )
            ...
            if self.fetch_content:
                ...
                self.log_signal.emit(
                    f"📖 第二阶段：抓取 {len(all_articles)} 篇正文", "info"
                )
                self.log_signal.emit(f"{'='*50}", "info")
                content_ok = self._crawl_contents(all_articles)
                self.log_signal.emit(
                    f"✅ 正文完成: {content_ok}/{len(all_articles)} 篇", "success"
                )

            if self._is_cancelled:
                self._db.finish_session(
                    session_id, len(all_articles), total_inserted, 'cancelled'
                )
                self.finished_signal.emit({"status": "cancelled"})
                return

            self._db.finish_session(
                session_id, len(all_articles), total_inserted, 'done'
            )
    ...
    def _generate_layout_urls(self):
        """根据起止日期生成所有版面列表的URL"""
        urls = []
        try:
            ...
            if start_dt > end_dt:
                self.log_signal.emit("❌ 起始日期不能晚于结束日期", "error")
                return []

            current_dt = start_dt
            while current_dt <= end_dt:
                # 格式化为 YYYYMM 和 D (不带前导0)
                ym = current_dt.strftime("%Y%m")
                d = str(current_dt.day)
                # 构造URL，例如：.../layout/202607/30/node_01.html
                url = f"{self.base_url}/layout/{ym}/{d}/node_01.html"
                urls.append(url)
                current_dt += timedelta(days=1)
        except ValueError:
            self.log_signal.emit("❌ 日期格式错误，应为 YYYY-MM-DD", "error")
        return urls

    def _extract_date_from_layout_url(self, url):
        """从版面URL中提取日期，例如 .../layout/202607/30/... -> 2026-07-30"""
        try:
            parts = url.split('/')
            ym = parts[-3] # 202607
            d = parts[-2]  # 30
            return f"{ym[:4]}-{ym[4:]}-{d.zfill(2)}"
        except Exception:
    ...
    # ---------- 解析版面页面 ----------
    def _parse_layout_page(self, html, layout_url, layout_date):
        """解析单个版面页面，提取文章列表"""
        ...
        # 提取版面名称
        section_name = "未知版面"
        # 尝试从页面中提取版面名称，例如从 title 或特定的 h1/h2 标签
        title_tag = soup.find('title')
        if title_tag:
            section_name = title_tag.get_text(strip=True).split('-')[0].strip()
        
        # 查找文章列表链接
        # 根据人民日报网页结构，文章链接通常在 ul.news-list 或类似的列表中
        links = soup.select('div.news ul.news-list li a[href]')
        if not links:
            # 备用选择器
        ...
        for a in links:
            ...
            articles.append({
                ...
                'subtitle': '',
                'author': '',
                'date': layout_date,  # 使用从版面URL提取的日期
    ...
    def _crawl_contents(self, articles):
        ...
        for idx, art in enumerate(articles):
            ...
            if not html:
                self.log_signal.emit("   ⚠️ 获取失败", "warn")
                continue
            ...
            if len(pending) >= self.batch_size:
                self._db.update_contents(pending)
                pending.clear()
    ...
    def _parse_article_page(self, html):
        ...
        for comment in soup.find_all(
            string=lambda t: t and 'enpproperty' in t
        ):
            ct = str(comment)
            ...
            if not result.get('title'):
                m = re.search(
                    r'<title><p>(.*?)</p></title>', ct, re.DOTALL
                )
                if m:
                    result['title'] = m.group(1).strip()
            if not result.get('subtitle'):
                m = re.search(
                    r'<introtitle><p>(.*?)</p></introtitle>', ct, re.DOTALL
                )
                if m:
...
class CrawlerTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.save_dir = self.settings.value(
            "save_dir", os.path.join(os.path.expanduser("~"), "CrawledArticles")
        )
    ...
    def init_ui(self):
        ...
        # 基础网址
        gl.addWidget(QLabel("基础网址:"), 0, 0)
        self.base_url_edit = QLineEdit(DEFAULT_BASE_URL)
        gl.addWidget(self.base_url_edit, 0, 1, 1, 2)
        
        # 日期范围
        gl.addWidget(QLabel("起始日期:"), 1, 0)
        self.start_date_edit = QDateEdit()
        self.start_date_edit.setCalendarPopup(True)
        self.start_date_edit.setDate(QDate.currentDate())
        self.start_date_edit.setDisplayFormat("yyyy-MM-dd")
        gl.addWidget(self.start_date_edit, 1, 1)
        
        gl.addWidget(QLabel("结束日期:"), 1, 2)
        self.end_date_edit = QDateEdit()
        self.end_date_edit.setCalendarPopup(True)
        self.end_date_edit.setDate(QDate.currentDate())
        self.end_date_edit.setDisplayFormat("yyyy-MM-dd")
        gl.addWidget(self.end_date_edit, 1, 3)

        # 目录
        gl.addWidget(QLabel("目录:"), 2, 0)
        self.path_edit = QLineEdit(self.save_dir)
        self.path_edit.setReadOnly(True)
        gl.addWidget(self.path_edit, 2, 1, 1, 2)
        btn_br = QPushButton("浏览...")
        btn_br.setFixedWidth(60)
        btn_br.clicked.connect(self.browse_folder)
        gl.addWidget(btn_br, 2, 3)
        ...
        self.btn_start.setStyleSheet("""
            QPushButton { background:#1976D2; color:white; font-weight:bold; padding:8px 20px; border-radius:4px; font-size:13px; }
        ...
        self.log_text.setStyleSheet("""
            QTextEdit { background:#FAFAFA; font-family:'Consolas','Microsoft YaHei'; font-size:12px; border:1px solid #E0E0E0; border-radius:4px; padding:6px; }
        """)
        ...
        self.stats_text.setStyleSheet("""
            QTextEdit { background:#F5F5F5; font-size:12px; border:1px solid #E0E0E0; border-radius:4px; padding:6px; }
        """)
    ...
    def show_export_menu(self):
        ...
        action = menu.exec_(self.btn_export.mapToGlobal(self.btn_export.rect().bottomLeft()))
        if not action:
            return
    ...
    def start_crawl(self):
        ...
        if not base_url:
            self.status_message.emit("⚠️ 请输入基础网址")
            return
        ...
        self.thread = CrawlerThread(
            base_url, start_date, end_date, self.db_path, 
            self.chk_content.isChecked(), BATCH_SIZE
        )
    ...
    def append_log(self, msg, level="info"):
        ...
        if self._log_count > MAX_LOG_LINES:
            self.log_text.clear()
            self._log_count = 0
            self.log_text.append('<span style="color:#999">--- 日志已截断 ---</span>')
        colors = {
            "info": "#333",
            "success": "#2E7D32",
            "error": "#C62828",
    ...
    def on_batch_saved(self, ins, skip):
        self.append_log(f"💾 批次写入: +{ins} 新增, {skip} 跳过(重复)", "info")
